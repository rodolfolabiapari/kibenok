...
void shift_quadrants(CvMat * img)
{
	...
	loop_swap: for (row = 0; row < half_row; row++) {
...
void normalize_merge (CvMat * mat) {
	...
	for (i = 0; i < mat->rows; i+=4) {
		for (j = 0; j < mat->cols; j+=4) {
			mat->data.ptr[i * mat->cols + j + 1] = 
					  mat->data.ptr[i * mat->cols + j + 0];
			mat->data.ptr[i * mat->cols + j + 2] = 
					  mat->data.ptr[i * mat->cols + j + 0];
			mat->data.ptr[i * mat->cols + j + 3] = 
					  mat->data.ptr[i * mat->cols + j + 0];
		}
	}
}
...
int main(void)
{
	...
	ipl_8_in_255 = (IplImage *) cvLoadImage(path, 
...
/*
	...
	printf("\nDoing the DFT.");
	// Does the DFT
	cvDFT(mat_64c2_plan, mat_64c2_plan, (CV_DXT_FORWARD), 0);  
	...
	printf("\nDoing the iDFT.");
	// Does the Inverse
	cvDFT(mat_64c2_plan, mat_64c2_plan , CV_DXT_INVERSE, 0);
